...
| `itdastream.admin.socket.enabled` | `true` | Whether the broker exposes the local Unix-domain admin recovery socket used by `bin/itdastream-cli.sh` (`ping`, `iam:reset-password`). Authentication is the socket's mode `600`; it is never exposed on the network. See [Admin Password Recovery](admin-password-recovery.md). |
...
| `itdastream.broker.drain.pause.ms` | `15000` | How long a broker keeps serving after announcing that it is draining, during a shutdown. Announcing removes it from the metadata every other broker hands to clients, but a client that fetched metadata a moment earlier still believes it is serving — this is the time it gets to notice. Set it to at least the clients' metadata refresh interval. See [Broker Drain](broker-drain.md). |
| `itdastream.broker.shutdown.grace.ms` | `30000` | How long shutdown waits for requests already being handled to finish before interrupting the handler threads. Interrupting mid-produce can abort a batch after it reached the staging store but before the write buffer took it, leaving a producer holding an acknowledgement for a record the log does not have. Bounded so a wedged handler cannot stop the broker stopping. |
