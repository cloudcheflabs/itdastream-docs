...
The 1.0.0 release closes this gap on **both ends** of every internal RPC — `InternalNioClient` (used by every broker → broker and broker → broker call) and `InternalNioServer` (the response side).
...
`com.cloudcheflabs.itdastream.protocol.internal.InternalNioClient`:
...
```text
-Ditdastream.protocol.internal.connect.timeout.seconds=10
-Ditdastream.protocol.internal.write.timeout.seconds=10
```

## What changed in the server

`com.cloudcheflabs.itdastream.protocol.internal.InternalNioServer`:
...
docker pause itdastream-worker-2          # silent NIC sim
./tests/test-nic-failure-channel-health.sh    # expect PASS
docker unpause itdastream-worker-2
```
