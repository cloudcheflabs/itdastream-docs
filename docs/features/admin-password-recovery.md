...
docker exec itdastream-broker /app/bin/itdastream-cli.sh iam:reset-password
```
...
```bash
NEW_PW=$(docker exec itdastream-broker /app/bin/itdastream-cli.sh iam:reset-password)
```
